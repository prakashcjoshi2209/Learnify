  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50">
      ...
      <div className="w-full md:w-3/4 p-4 md:p-8">
        <h2 className="text-3xl md:text-4xl font-extrabold text-gray-800 mb-4 md:mb-6 text-center md:text-left">
          Shopping Cart
        </h2>
        ...
        {cart.length === 0 ? (
          ...
          <div className="space-y-4 md:space-y-6">
            {cart.map((item) => (
              <div
                key={item._id}
                className="relative flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 bg-white p-4 md:p-6 rounded-lg shadow-md border-l-4 border-purple-500 hover:shadow-lg transition-transform hover:-translate-y-1 duration-300"
              >
                {/* Remove Button */}
                {isRemoving ? (
                  <ClipLoader size={30} color="red" />
                  ...
                  <button
                    onClick={() => handleRemove(item.courseId)}
                    className="absolute top-3 right-3  md:top-3 md:right-3 text-red-500 hover:text-red-700 transition"
                  >
                    <TrashIcon className="w-6 h-6  md:w-6 md:h-6" />
                  </button>
                )}
                ...
                <div className="text-center md:text-right mt-6">
                  ...
                  <button
                    ...
                    className={`mt-3 px-4 py-2 w-full md:w-auto text-white rounded-lg transition shadow-md ${
                      processingCourse === item.courseId
                        ? "bg-gray-400 cursor-not-allowed"
                        : "bg-gradient-to-r from-purple-500 to-indigo-600 hover:bg-indigo-700"
                    }`}
                    disabled={processingCourse === item.courseId}
                  >
                    {processingCourse === item.courseId
        ...
        {/* Total Section */}
        {cart.length > 0 && (
          <div className="mt-6 md:mt-8 p-4 md:p-6 bg-white shadow-md rounded-lg flex flex-col md:flex-row justify-between items-center border-t-4 border-blue-500">
            ...
            <button
              ...
              className="mt-3 md:mt-0 px-5 py-3 w-full md:w-auto bg-gradient-to-r from-purple-500 to-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition shadow-md"
            >
              {isBuyingAll ? "Processing..." : "Checkout"}
            </button>
          </div>
